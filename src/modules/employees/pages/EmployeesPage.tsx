...
import { getEmployees, deleteEmployee, updateEmployee } from "../services/employee.service";
import type { Employee } from "../models/employee.model";
import { useHeaderAction, CardEmptyAdded, Modal } from "@/modules/shared";
...
const EmployeesPage = () => {
  ...
  const handleDelete = async () => {
    if (!employeeToDelete || !restaurantId) return;
    try {
      await deleteEmployee(restaurantId, employeeToDelete);
  ...
  const handleEditRole = async () => {
    if (!employeeToEdit || !restaurantId) return;
    try {
      await updateEmployee(restaurantId, employeeToEdit.id, { role: newRole });
      setEmployees(prev => prev.map(e => e.id === employeeToEdit.id ? { ...e, role: newRole } : e));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <>
      ...
      >
        <p className="text-gray-600 dark:text-gray-400">¿Estás seguro de que deseas eliminar a este empleado? Esta acción no se puede deshacer.</p>
      </Modal>
      ...
      >
        <div className="flex flex-col gap-3">
          <p className="text-gray-600 dark:text-gray-400 text-sm">Selecciona el nuevo rol para <strong>{employeeToEdit?.name}</strong>:</p>
