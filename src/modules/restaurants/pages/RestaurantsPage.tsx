      ...
      <Container>
        ...
        {loading ? "" : !isCollaboratorOnly && (
          <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6 mb-8 md:mb-12">
            ...
            <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-md border border-white/50 dark:border-white/5 p-3 md:p-6 rounded-xl md:rounded-2xl shadow-sm hover:shadow-md transition flex flex-col items-center md:items-start text-center md:text-left">
              <div className="flex flex-col md:flex-row w-full justify-between items-center md:items-start mb-2 md:mb-4 gap-2">
                <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-keleo-50 dark:bg-keleo-900/20 text-keleo-600 flex items-center justify-center group-hover:scale-110 transition mx-auto md:mx-0">
                  <Coins className="w-4 h-4 md:w-6 md:h-6" />
                </div>
                {(stats?.sales.percentageComparation || 0) >= 0 ? (
                  <span className="text-[9px] md:text-xs font-bold text-green-500 bg-green-100 dark:bg-green-900/30 px-1.5 py-0.5 md:px-2 md:py-1 rounded flex items-center gap-0.5">
                    <ArrowUp className="w-2 h-2 md:w-3.5 md:h-3.5" /> {stats?.sales.percentageComparation || 0}%
                  </span>
                ) : (
                  <span className="text-[9px] md:text-xs font-bold text-red-500 bg-red-100 dark:bg-red-900/30 px-1.5 py-0.5 md:px-2 md:py-1 rounded flex items-center gap-0.5">
                    <ArrowDown className="w-2 h-2 md:w-3.5 md:h-3.5" /> {stats?.sales.percentageComparation || 0}%
                  </span>
                )}
              </div>
              <p className="text-[9px] md:text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mb-0.5 md:mb-1 truncate w-full">
                <span className="md:hidden">Ventas</span>
                <span className="hidden md:inline">Ventas Consolidadas</span>
              </p>
              <h3 className="text-sm sm:text-base md:text-3xl font-bold text-gray-900 dark:text-white truncate w-full">
            ...
            <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-md border border-white/50 dark:border-white/5 p-3 md:p-6 rounded-xl md:rounded-2xl shadow-sm hover:shadow-md transition flex flex-col items-center md:items-start text-center md:text-left">
              <div className="flex flex-col md:flex-row w-full justify-between items-center md:items-start mb-2 md:mb-4 gap-2">
                <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-orange-50 dark:bg-orange-900/20 text-orange-500 flex items-center justify-center group-hover:scale-110 transition mx-auto md:mx-0">
                  <Tag className="w-4 h-4 md:w-6 md:h-6" />
                </div>
                <span className="flex items-center gap-1 text-[9px] md:text-xs font-bold text-orange-600 animate-pulse">
            ...
            {/* Stat 3: Staff */}
            <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-md border border-white/50 dark:border-white/5 p-3 md:p-6 rounded-xl md:rounded-2xl shadow-sm hover:shadow-md transition flex flex-col items-center md:items-start text-center md:text-left">
              <div className="flex flex-col md:flex-row w-full justify-between items-center md:items-start mb-2 md:mb-4">
                <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-500 flex items-center justify-center group-hover:scale-110 transition mx-auto md:mx-0">
                  <Users className="w-4 h-4 md:w-6 md:h-6" />
                </div>
              </div>
              <p className="text-[9px] md:text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mb-0.5 md:mb-1 truncate w-full">
                <span className="md:hidden">Staff</span>
                <span className="hidden md:inline">Equipo en Turno</span>
              </p>
              <h3 className="text-sm sm:text-base md:text-3xl font-bold text-gray-900 dark:text-white truncate w-full">
                {stats?.staff.active || 0} <span className="text-[10px] md:text-sm font-normal text-gray-400">/ {stats?.staff.total || 0}</span>
              </h3>
            </div>
